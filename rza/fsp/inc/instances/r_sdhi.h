...
typedef struct st_sdmmc_instance_ctrl
{
    ...
    void                * p_buff;                 // Pointer to uncached transfer buffer
    uint64_t              buff_physical_address;  // Phisical address of uncached transfer buffer
...
typedef struct st_sdhi_extended_cfg
{
    ...
    IRQn_Type                oxasioirq_irq;    ///< OXASIOIRQ IRQ number
    uint8_t                  oxmnirq_ipl;      ///< OXMNIRQ interrupt priority
    uint8_t                  oxasioirq_ipl;    ///< OXASIOIRQ interrupt priority
} sdhi_extended_cfg_t;
