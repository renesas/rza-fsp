...
#define SDHI_PRV_SDHI_INFO1_CARD_MASK                      ((SDHI_PRV_SDHI_INFO1_CARD_REMOVED | \
                                                             SDHI_PRV_SDHI_INFO1_CARD_INSERTED)) // When the DAT3 line is pulled up with UHS-I support,
#define SDHI_PRV_SDHI_INFO1_CARD_REMOVED_MASK              ((SDHI_PRV_SDHI_INFO1_CARD_REMOVED))  // the insertion/removal of a card using the DAT3 line cannot be detected,
#define SDHI_PRV_SDHI_INFO1_CARD_INSERTED_MASK             ((SDHI_PRV_SDHI_INFO1_CARD_INSERTED)) // so the detection specification for the DAT3 line has been deleted.
...
#define SDHI_PRV_SD_CLK_CTRL_DEFAULT                       (0x40U) // Due to the increase in the clock frequency supplied to the module,
                                                                   // the division ratio was changed to make SD_CLK in the range of 100KHz to 400KHz.
...
#define SDHI_PRV_SD_INFO1_MASK_MASK_ACEND_ENABLE           (0x319U)  // Added mask data to detect only end of access.
...
static fsp_err_t r_sdhi_read_write_common(sdhi_instance_ctrl_t * const p_ctrl,
...
static void r_sdhi_dma_prepare(sdhi_instance_ctrl_t * p_ctrl, bool is_write, uint64_t pa);
...
/* Uncached transfer buffer */
static uint8_t uncached_buf[BSP_FEATURE_SDHI_MAX_CHANNELS][SDHI_MAX_BLOCK_SIZE] __attribute__((section("UNCACHED_BSS"),
                                                                                               aligned(
                                                                                                   SDHI_PRV_BUFF_ALIGN)));
...
fsp_err_t R_SDHI_Open (sdmmc_ctrl_t * const p_api_ctrl, sdmmc_cfg_t const * const p_cfg)
{
    ...
    /* Obtaining transfer buffer */
    p_ctrl->p_buff = &uncached_buf[p_cfg->channel][0];
    err            = R_MMU_VAtoPA(NULL, (uintptr_t) p_ctrl->p_buff, &p_ctrl->buff_physical_address);
    if (FSP_SUCCESS != err)
    {
        return err;
    }
#else
    p_ctrl->buff_physical_address = p_ctrl->p_buff;
#endif
    /* Enable the SDHI channel and reset the registers to their initial state. */
    R_BSP_MODULE_START(FSP_IP_SDHI, p_cfg->channel);
    R_BSP_MODULE_CLKON(FSP_IP_SDHI, p_cfg->channel);
    R_BSP_MODULE_RSTOFF(FSP_IP_SDHI, p_cfg->channel);

    /* Reset stale interrupt flags */
    p_ctrl->p_reg->SD_INFO1_long.L = 0U;

    /* Reset SDHI. */
    p_ctrl->p_reg->SOFT_RST_long.L = 0x0U;
    p_ctrl->p_reg->SOFT_RST_long.L = 0x1U;

    /* Configure card detection. */
    if (SDMMC_CARD_DETECT_CD == p_ctrl->p_cfg->card_detect)
    {
        p_ctrl->p_reg->SD_INFO1_MASK_long.L = SDHI_PRV_SD_INFO1_MASK_CD_ENABLE;
    }
    else
    {
        p_ctrl->p_reg->SD_INFO1_MASK_long.L = SDHI_PRV_SD_INFO1_MASK_MASK_ALL;
    }
...
#if SDHI_CFG_SDIO_SUPPORT_ENABLE
    r_sdhi_irq_enable(p_extend->oxasioirq_irq, p_extend->oxasioirq_ipl, p_ctrl);
#endif
                            ...
                            sdmmc_io_address_mode_t  address_mode)
{
    ...
    for (uint32_t i = 0; i < block_count; i++)
    {
        ...
        err = r_sdhi_read_write_common(p_ctrl, 1U, byte_count, command, argument.arg);
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
fsp_err_t R_SDHI_IoIntEnable (sdmmc_ctrl_t * const p_api_ctrl, bool enable)
{
    ...
    FSP_ERROR_RETURN(SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_VAL ==
                     (p_ctrl->p_reg->SD_INFO2_long.L & SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_MASK),
                     FSP_ERR_DEVICE_BUSY);

    /* Enable or disable interrupt. */
    if (enable)
    {
        p_ctrl->p_reg->SDIO_MODE_long.L       = 1U;
        p_ctrl->p_reg->SDIO_INFO1_MASK_long.L = 0x6U;
    }
    else
    {
        p_ctrl->p_reg->SDIO_MODE_long.L       = 0U;
        p_ctrl->p_reg->SDIO_INFO1_MASK_long.L = SDHI_PRV_SDIO_INFO1_MASK_IRQ_DISABLE;
    }
...
fsp_err_t R_SDHI_Close (sdmmc_ctrl_t * const p_api_ctrl)
{
    ...
    const sdhi_extended_cfg_t * p_extend = (const sdhi_extended_cfg_t *) p_ctrl->p_cfg->p_extend;
    r_sdhi_irq_disable(p_extend->oxmnirq_ipl);
#if SDHI_CFG_SDIO_SUPPORT_ENABLE
    r_sdhi_irq_disable(p_extend->oxasioirq_ipl);
#endif
    ...
    R_BSP_MODULE_RSTON(FSP_IP_SDHI, p_ctrl->p_cfg->channel);
    R_BSP_MODULE_CLKOFF(FSP_IP_SDHI, p_ctrl->p_cfg->channel);
    R_BSP_MODULE_STOP(FSP_IP_SDHI, p_ctrl->p_cfg->channel);
...
static fsp_err_t r_sdhi_erase_error_check (sdhi_instance_ctrl_t * const p_ctrl,
                                           ...
                                           uint32_t const               sector_count)
{
#if SDHI_CFG_PARAM_CHECKING_ENABLE && SDHI_CFG_SD_SUPPORT_ENABLE
...
static void r_sdhi_access_irq_process (sdhi_instance_ctrl_t * p_ctrl, sdmmc_callback_args_t * p_args)
{
    ...
    info1 = p_ctrl->p_reg->SD_INFO1_long.L;
    info2 = p_ctrl->p_reg->SD_INFO2_long.L;
    info3 = p_ctrl->p_reg->DM_CM_INFO1_long.L;
    info4 = p_ctrl->p_reg->DM_CM_INFO2_long.L;

    /* Clear interrupt flags processed in this ISR. */
    info1 &= (SDHI_PRV_SDHI_INFO1_ACCESS_MASK | SDHI_PRV_SDHI_INFO1_CARD_MASK);
    info2 &= SDHI_PRV_SDHI_INFO2_MASK;
    info3 &= SDHI_PRV_SDHI_DM_CM_INFO1_MASK;
    info4 &= SDHI_PRV_SDHI_DM_CM_INFO2_MASK;
    p_ctrl->p_reg->SD_INFO1_long.L    = (~info1);
    p_ctrl->p_reg->SD_INFO2_long.L    = (~info2);
    p_ctrl->p_reg->DM_CM_INFO1_long.L = (~info3);
    p_ctrl->p_reg->DM_CM_INFO2_long.L = (~info4);

    /* Combine all flags in one 32 bit word. */
    flags.word = (info1 | (info2 << 16));

    if (0U != (flags.word & SDHI_PRV_SDHI_INFO1_CARD_REMOVED_MASK))
    {
        p_ctrl->initialized = false;
        p_args->event      |= SDMMC_EVENT_CARD_REMOVED;
    }

    if (0U != (flags.word & SDHI_PRV_SDHI_INFO1_CARD_INSERTED_MASK))
    {
    ...
    if (flags.bit.response_end)
    {
        ...
        if (1U == p_ctrl->p_reg->SD_STOP_b.SEC)
        {
            ...
            p_args->response.status = p_ctrl->p_reg->SD_RSP54_long.L;
        }
        else
        {
            p_args->response.status = p_ctrl->p_reg->SD_RSP10_long.L;
        }

        if (SDHI_PRV_CMD_ERASE == p_ctrl->p_reg->SD_CMD_long.L)
        {
        ...
        else
        {
            ...
            uint32_t mask = p_ctrl->p_reg->SD_INFO1_MASK_long.L;
            mask &= (~SDHI_PRV_SDHI_INFO1_ACCESS_END);
            mask |= SDHI_PRV_SDHI_INFO1_RESPONSE_END;
            p_ctrl->p_reg->SD_INFO1_MASK_long.L = mask;
        }
    }

    /* Check for errors */
    if ((flags.word & SDHI_PRV_ACCESS_ERROR_MASK) || info4)
    {
        flags.bit.event_error         = 1U;
        p_args->event                |= SDMMC_EVENT_TRANSFER_ERROR;
        p_ctrl->p_reg->SD_STOP_long.L = 1U;
...
static void r_sdhi_command_send_no_wait (sdhi_instance_ctrl_t * p_ctrl, uint32_t command, uint32_t argument)
{
    /* Clear Status */
    p_ctrl->p_reg->SD_INFO1_long.L = 0U;
    p_ctrl->p_reg->SD_INFO2_long.L = 0U;
    ...
    mask |= SDHI_PRV_SDHI_INFO1_ACCESS_END;
    p_ctrl->p_reg->SD_INFO1_MASK_long.L = mask;
    p_ctrl->p_reg->SD_INFO2_MASK_long.L = SDHI_PRV_SDHI_INFO2_MASK_CMD_SEND | R_SDHI_SD_INFO2_BRE_Msk |
                                          R_SDHI_SD_INFO2_BWE_Msk; // Added the setting to support SDHI built-in DMAC.

    /* Enable Clock */
    p_ctrl->p_reg->SD_CLK_CTRL_long.L |= SDHI_PRV_SDHI_PRV_SD_CLK_AUTO_CLOCK_ENABLE_MASK;

    /* Write argument, then command to the SDHI peripheral. */
    p_ctrl->p_reg->SD_ARG_long.L  = argument & UINT16_MAX;
    p_ctrl->p_reg->SD_ARG1_long.L = argument >> 16;
    p_ctrl->p_reg->SD_CMD_long.L  = command;
}
...
static fsp_err_t r_sdhi_max_clock_rate_set (sdhi_instance_ctrl_t * p_ctrl, uint32_t max_rate)
{
    ...
    {
        frequency = R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_SD0CLK) / 4; // Clock frequency for SDHI ch 0
    }
    else
    {
        frequency = R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_SD1CLK) / 4; // Clock frequency for SDHI ch 1
    }
        ...
        if ((frequency >> divisor_shift) <= max_rate)
        {
            ...
            while (timeout > 0U)
            {
                ...
                if (p_ctrl->p_reg->SD_INFO2_b.SCLKDIVEN)
                {
                    ...
                    uint32_t clkctrlen = p_ctrl->p_reg->SD_CLK_CTRL_long.L &
                                         SDHI_PRV_SDHI_PRV_SD_CLK_CTRL_CLKCTRLEN_MASK;
                    p_ctrl->p_reg->SD_CLK_CTRL_long.L = setting | clkctrlen | SDHI_PRV_SDHI_PRV_SD_CLK_CTRL_CLKEN_MASK;
...
static fsp_err_t r_sdhi_hw_cfg (sdhi_instance_ctrl_t * const p_ctrl)
{
    ...
    p_ctrl->p_reg->SD_OPTION_long.L = SDHI_PRV_SD_OPTION_DEFAULT |
     ...
     * Automatic Control of SD/MMC Clock Output (SD/MMC) of the RA6M3 manual R01UH0886EJ0110. */
    p_ctrl->p_reg->SD_CLK_CTRL_long.L = SDHI_PRV_SD_CLK_CTRL_DEFAULT | SDHI_PRV_SDHI_PRV_SD_CLK_CTRL_CLKCTRLEN_MASK; // Automatic clock control enabled.
...
static fsp_err_t r_sdhi_bus_cfg (sdhi_instance_ctrl_t * const p_ctrl)
{
    ...
    if (p_ctrl->device.card_type == SDMMC_CARD_TYPE_SDIO)
    {
        ...
        p_ctrl->p_reg->SD_OPTION_long.L = SDHI_PRV_SD_OPTION_DEFAULT | (bus_width_reg << SDHI_PRV_SD_OPTION_WIDTH8_BIT);

        /* Enable SDIO interrupts. Busy flag must be cleared before enabling interrupts. */
        p_ctrl->p_reg->SDIO_MODE_b.IOMOD      = 1U;
        p_ctrl->p_reg->SDIO_INFO1_MASK_long.L = 0x6U;
    }
    else
#endif
                                           ...
                                           uint32_t                     argument)
{
    ...
    if (sector_count > 1U)
    {
        p_ctrl->p_reg->SD_STOP_long.L   = SDHI_PRV_SD_STOP_SD_SECCNT_ENABLE;
        p_ctrl->p_reg->SD_SECCNT_long.L = sector_count;
    }
    else
    {
        p_ctrl->p_reg->SD_STOP_long.L = 0U;
    }

    /* Set sector size */
    p_ctrl->p_reg->SD_SIZE_long.L = sector_size;

    /* Send command. */
    return r_sdhi_command_send(p_ctrl, command, argument);
}
                                  ...
                                  sdmmc_priv_csd_reg_t * const p_csd_reg)
{
    ...
    p_csd_reg->reg.sdrsp10 = p_ctrl->p_reg->SD_RSP10_long.L;
    p_csd_reg->reg.sdrsp32 = p_ctrl->p_reg->SD_RSP32_long.L;
    p_csd_reg->reg.sdrsp54 = p_ctrl->p_reg->SD_RSP54_long.L;
    p_csd_reg->reg.sdrsp76 = p_ctrl->p_reg->SD_RSP76_long.L;
...
{
    /* Issue command. */
    fsp_err_t err = r_sdhi_read_write_common(p_ctrl, 1U, byte_count, command, argument);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    ...
    /* Copy driver buffer data to caller buffer */
    memcpy(&p_ctrl->aligned_buff[0], p_ctrl->p_buff, byte_count);
...
static void r_sdhi_transfer_end (sdhi_instance_ctrl_t * const p_ctrl)
{
    ...
    p_ctrl->p_reg->DM_CM_DTRAN_CTRL_long.L = 0U;
    p_ctrl->p_reg->CC_EXT_MODE_long.L      = 0U;
}

/*******************************************************************************************************************//**
 * Preparation for DMA transfer.
 *
 * @param[in]  p_ctrl               Pointer to the instance control block.
 * @param[in]  is_write             Transfer direction specification.
 * @param[in]  pa                   Physical address for DMA transfer.
 **********************************************************************************************************************/
static void r_sdhi_dma_prepare (sdhi_instance_ctrl_t * p_ctrl, bool is_write, uint64_t pa)
{
    p_ctrl->p_reg->SD_INFO1_MASK_long.L = SDHI_PRV_SD_INFO1_MASK_MASK_ACEND_ENABLE;
    p_ctrl->p_reg->SD_INFO2_MASK_long.L = SDHI_PRV_SDHI_INFO2_MASK;
    p_ctrl->p_reg->CC_EXT_MODE_long.L   = SDHI_PRV_SD_DMAEN_DMAEN_SET;
    if (is_write)
    {
        p_ctrl->p_reg->DM_CM_INFO1_MASK_long.L = ~(SDHI_PRV_SDHI_DM_CM_INFO1_SEQEND |
                                                   SDHI_PRV_SDHI_DM_CM_INFO1_DTRANEND0);
        p_ctrl->p_reg->DM_CM_INFO2_MASK_long.L = ~(SDHI_PRV_SDHI_DM_CM_INFO2_SEQERR |
                                                   SDHI_PRV_SDHI_DM_CM_INFO2_DTRANERR0);

        p_ctrl->p_reg->DM_CM_DTRAN_MODE_long.L = SDHI_PRV_SDHI_DM_CM_DTRAN_MODE_BUS_WIDTH |
    ...
    p_ctrl->p_reg->DM_DTRAN_ADDR_long.L = (uint32_t) pa;
}
...
void sdhimmc_oxmnirq_isr (IRQn_Type const irq)
{
    ...
    if ((p_ctrl->initialized) &&
        (0U != (args.event & (uint32_t) ~(SDMMC_EVENT_CARD_REMOVED | SDMMC_EVENT_CARD_INSERTED))))
    {
        r_sdhi_call_callback(p_ctrl, &args);
    }
    else if (0U != (args.event & (SDMMC_EVENT_CARD_REMOVED | SDMMC_EVENT_CARD_INSERTED)))
    {
        args.p_context = p_ctrl->p_context;
        r_sdhi_call_callback(p_ctrl, &args);
    }
...
void sdhimmc_oxasioirq_isr (IRQn_Type const irq)
{
    ...
    sdmmc_callback_args_t args;
    memset(&args, 0U, sizeof(args));
    uint32_t info1 = p_ctrl->p_reg->SDIO_INFO1_long.L;
    ...
    p_ctrl->p_reg->SDIO_INFO1_long.L = SDHI_PRV_SDIO_INFO1_IRQ_CLEAR;
