...
usb_msc_cbw_t      g_usb_hmsc_cbw[USB_NUM_USBIP][USB_MAXSTRAGE] USB_BUFFER_PLACE_IN_SECTION; /* CBW headder */
usb_msc_csw_t      g_usb_hmsc_csw[USB_NUM_USBIP][USB_MAXSTRAGE] USB_BUFFER_PLACE_IN_SECTION; /* CSW headder */
        ...
        if (USB_DT_ENDPOINT == table[ofdsc + 1])                     /* Search within Interface */
        {
            if (USB_EP_BULK == (table[ofdsc + 3] & USB_EP_TRNSMASK)) /* Bulk Endpoint */
            {
                ...
                if (USB_EP_IN == (pdescriptor[2] & USB_EP_DIRMASK))
                {
                    ...
                    g_usb_hmsc_in_pipe[ip][side] = (uint8_t) R_USB_HstdGetPipeID(devadr.keyword, epnum);
                }
                else
                {
                    ...
                    g_usb_hmsc_out_pipe[ip][side] = (uint8_t) R_USB_HstdGetPipeID(devadr.keyword, epnum);
                }
...
static void usb_hmsc_detach (usb_utr_t * ptr, uint16_t addr, uint16_t data2)
{
    ...
    {
        if (1 == R_USB00->PORTSC1_b.PortOwner)
        {
            p_cfg = (usb_cfg_t *) R_FSP_IsrContextGet((IRQn_Type) USB_U2H0_OHCI_INT_IRQn);
        }
        else
        {
