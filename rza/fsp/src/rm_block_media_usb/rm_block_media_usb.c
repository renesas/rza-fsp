...
fsp_err_t RM_BLOCK_MEDIA_USB_MediaInit (rm_block_media_ctrl_t * const p_ctrl)
{
    ...
    memset(&state_info, 0, sizeof(usb_info_t));
...
#if (1)
    p_instance_ctrl->sector_count = (uint32_t) (p_instance_ctrl->p_read_buffer[0] << 24) |
                                    (uint32_t) (p_instance_ctrl->p_read_buffer[1] << 16) |
                                    (uint32_t) (p_instance_ctrl->p_read_buffer[2] << 8) |
                                    (uint32_t) (p_instance_ctrl->p_read_buffer[3] << 0);
    p_instance_ctrl->sector_count++;
    p_instance_ctrl->sector_size_bytes = (uint32_t) (p_instance_ctrl->p_read_buffer[4] << 24) |
                                         (uint32_t) (p_instance_ctrl->p_read_buffer[5] << 16) |
                                         (uint32_t) (p_instance_ctrl->p_read_buffer[6] << 8) |
                                         (uint32_t) (p_instance_ctrl->p_read_buffer[7] << 0);
#else
    p_instance_ctrl->sector_count      = (*(uint32_t *) &p_instance_ctrl->p_read_buffer[0]) + 1;
    p_instance_ctrl->sector_size_bytes = (*(uint32_t *) &p_instance_ctrl->p_read_buffer[4]);
#endif
 ...
 *
 * @retval     FSP_SUCCESS                        Updated the user callback to provide memory successfully.
 * @retval     FSP_ERR_ASSERTION                  NULL pointer.
 * @retval     FSP_ERR_NOT_OPEN                   Module is not open.
 * @retval     FSP_ERR_NO_CALLBACK_MEMORY         Non-secure callback memory not provided for non-secure callback.
 *
 * @return See @ref RENESAS_ERROR_CODES or functions called by this function for other possible return codes.
 **********************************************************************************************************************/
