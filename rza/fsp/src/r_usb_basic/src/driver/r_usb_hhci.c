...
#define USB_HCI_BAREMETAL_RCV_ERROR    (1)
#define USB_HCI_RTOS_RCV_ERROR         (2)
...
void usb_hstd_hci_interrupt_handler(usb_utr_t * ptr);
...
static uint32_t usb_hstd_hci_get_port_current_connect_status(usb_utr_t * ptr);
...
void usb_hstd_hci_task (void)
{
    ...
    if (err != USB_OK)
    {
        result = USB_HCI_BAREMETAL_RCV_ERROR;
    }
    else
    {
    ...
    if (result == USB_HCI_RTOS_RCV_ERROR)
    {
        result = 0;
        continue;
    }
...
void usb_hstd_hci_interrupt_handler (usb_utr_t * ptr)
{
    ...
    if (USB_IP0 == ptr->ip)
    {
        ...
        if (ptr->ipp->HCINTERRUPTSTATUS_b.RHSC && ptr->ipp->HCINTERRUPTENABLE_b.RHSCE)
        {
            usb_hcrh_descriptor_a = ptr->ipp->HCRHDESCRIPTORA;
            usb_hcrh_descriptor_b = ptr->ipp->HCRHDESCRIPTORB;
            usb_portsc1           = ptr->ipp->PORTSC1;
            usb_hcrh_port_status1 = ptr->ipp->HCRHPORTSTATUS1;
 ...
 #else
        else if (ptr->ipp->USBSTS_b.PTCGDET && ptr->ipp->USBINTR_b.PTCGIE)
        {
            if (0 == ptr->ipp->HCRHPORTSTATUS1_b.CCS)
            {
                /* R_USB_HstdDelayXms((uint16_t)30); */
    ...
    else
    {
        ...
        {
            usb_hcrh_descriptor_a = ptr->ipp1->HCRHDESCRIPTORA;
            usb_hcrh_descriptor_b = ptr->ipp1->HCRHDESCRIPTORB;
            usb_portsc1           = ptr->ipp1->PORTSC1;
        ...
        else if (ptr->ipp1->USBSTS_b.PortChangeDetect && ptr->ipp1->USBINTR_b.PortChangeDetectEnable)
        {
            if (0 == ptr->ipp1->HCRHPORTSTATUS1_b.CCS)
            {
                /* R_USB_HstdDelayXms((uint16_t)30); */
    ...
    g_usb_cstd_int_msg_cnt[ptr->ip]++;
    if (USB_INTMSGMAX == g_usb_cstd_int_msg_cnt[ptr->ip])
    {
...
/* USB_SND_MSG(USB_HCI_MBX, (usb_msg_t *)NULL); */
 #if (BSP_CFG_RTOS == 1)
    err = USB_SND_MSG(USB_HCI_MBX, (usb_msg_t *) ptr);
    if (err != USB_OK)
    {
        /* Send Message Error  */
        USB_HCI_PRINTF0_R("Error: HCI send message Error \n");
    }
 #else
    err = USB_ISND_MSG(USB_HCI_MBX, (usb_msg_t *) ptr);
    if (err != USB_OK)
    {
        /* Send Message Error  */
        USB_HCI_PRINTF0_R("Error: HCI send message Error \n");
 ...
 * Note         :
 ***********************************************************************************************************************/
int32_t r_usb_hstd_hci_init (usb_utr_t * ptr, st_usb_hci_cb_info_t * p_cb_info)
{
    uint32_t err;
                                               ...
                                               uint32_t mps)
{
    ...
    switch (p_tr_req->devinfo->bit.hctype)
    {
        ...
        {
            usb_hstd_ohci_make_transfer_request(p_tr_req);
            break;
        }
...
void r_usb_hstd_hci_disconnect (usb_utr_t * ptr, uint32_t devadd)
{
    ...
    if (NULL != p_devinfo)
    {
        ...
        switch (p_devinfo->bit.hctype)
        {
            case USB_HCI_HCTYPE_EHCI:
            {
                usb_hstd_ehci_disconnect(ptr, devadd);
            ...
            case USB_HCI_HCTYPE_OHCI:
            {
                if (USB_IP0 == ptr->ip)
                {
                    ptr->ipp->HCCONTROL = ptr->ipp->HCCONTROL | (USB_VAL_XC0 & USB_VAL_XFFCF);
                    ...
                    ptr->ipp->HCCONTROL = (ptr->ipp->HCCONTROL & ~USB_VAL_XC0) | USB_VAL_X0080;
                }
                else
                {
...
static uint32_t usb_hstd_hci_get_port_current_connect_status (usb_utr_t * ptr)
{
    return usb_hstd_ehci_get_port_current_connect_status(ptr);
...
void r_usb_hstd_hci_port_reset (usb_utr_t * p_utr)
{
    ...
    if (USB_HCI_NODEVICE != devaddr)
    {
        usb_hstd_ehci_port_reset(p_utr);
    ...
    else
    {
        ...
        if (USB_HCI_NODEVICE != devaddr)
        {
            usb_hstd_ohci_port_reset(p_utr);
            g_usb_hstd_hci_callback.port_reset_cb(p_utr);
        }
    }
 ...
 * Note         :
 ***********************************************************************************************************************/
void r_usb_hstd_hci_port_suspend (usb_utr_t * ptr)
{
    uint16_t devaddr;

    devaddr = usb_hstd_ehci_get_device_address_of_rootpoot(ptr->ip);
    if (USB_HCI_NODEVICE != devaddr)
    {
        usb_hstd_ehci_port_suspend(ptr);
    }
    else
    {
        devaddr = usb_hstd_ohci_get_device_address_of_rootpoot(ptr->ip);
        if (USB_HCI_NODEVICE != devaddr)
        {
            usb_hstd_ohci_port_suspend(ptr);
        }
    }
 ...
 * Note         :
 ***********************************************************************************************************************/
void r_usb_hstd_hci_port_resume (usb_utr_t * ptr)
{
    uint16_t devaddr;
...
/*********************/
 #ifdef USB_HOST_COMPLIANCE_MODE
void r_usb_hstd_hci_electrical_test (usb_utr_t * ptr, uint8_t mode)
{
    switch (mode)
    {
        case 0:                        /* Test SE0_NAK */
        {
            ...
            {
                ptr->ipp1.PORTSC1_b.PortTestControl = 3;
            }
  #else
            ptr->ipp->PORTSC1_b.PTTST = 3;
  #endif
            while (1)
            {
        ...
        case 1:                        /* Test J */
        {
            ...
            {
                ptr->ipp1.PORTSC1_b.PortTestControl = 1;
            }
  #else
            ptr->ipp->PORTSC1_b.PTTST = 1;
  #endif
            while (1)
            {
                ;                      /* This loops infinitely until it's reset. */
        ...
        case 2:                        /* Test K */
        {
            ...
            {
                ptr->ipp1.PORTSC1_b.PortTestControl = 2;
            }
  #else
            ptr->ipp->PORTSC1_b.PTTST = 2;
  #endif
            while (1)
            {
                ;                      /* This loops infinitely until it's reset. */
        ...
        case 3:                        /* Signal Quality */
        {
            ...
            {
                ptr->ipp1.PORTSC1_b.PortTestControl = 4;
            }
  #else
            ptr->ipp->PORTSC1_b.PTTST = 4;
  #endif
            while (1)
            {
                ;                      /* This loops infinitely until it's reset. */
        ...
        case 4:                        /* Suspend/Resume */
        {
            ...
            if (USB_IP0 == ptr->ip)
            {
                ...
                ptr->ipp.PORTSC1_b.Suspend = 1;         /* Suspend */
                usb_cpu_delay_xms(15000);
                ptr->ipp.PORTSC1_b.ForcePortResume = 1; /* Resume start */
                usb_cpu_delay_xms(20);
                ptr->ipp.PORTSC1_b.ForcePortResume = 0; /* Resume end */
            }
            else
            {
                usb_cpu_delay_xms(15000);
                ptr->ipp1.PORTSC1_b.Suspend = 1;         /* Suspend */
                usb_cpu_delay_xms(15000);
                ptr->ipp1.PORTSC1_b.ForcePortResume = 1; /* Resume start */
                usb_cpu_delay_xms(20);
                ptr->ipp1.PORTSC1_b.ForcePortResume = 0; /* Resume end */
            }
  #else
            usb_cpu_delay_xms(15000);                    /* wait 15sec */
            ptr->ipp->PORTSC1_b.SUSPEND = 1;             /* Suspend */
            usb_cpu_delay_xms(15000);                    /* wait 15sec */
            ptr->ipp->PORTSC1_b.FRCPTRSM = 1;            /* Resume start */
            usb_cpu_delay_xms(20);                       /* wait 20ms */
            ptr->ipp->PORTSC1_b.FRCPTRSM = 0;            /* Resume end */
  #endif
            break;
        }
        ...
        {
            usb_hstd_electrical_test_mode(0x0108, ptr);
            break;
        }
        ...
        case 6:                        /* Chirp Timing */
        {
            ...
            if (USB_IP0 == ptr->ip)
            {
                ...
                if (1 == ptr->ipp.PORTSC1.BIT.CurrentConnectStatus)
                {
                    ptr->ipp->USBINTR = 0x00000000; /* Disable interrupt */
                ...
                ptr->ipp->USBINTR = 0x00000007; /* PortChange, USBError, USBInt */
            }
            else
            {
                ptr->ipp1.PORTSC1.BIT.Suspend = 1;
                if (1 == ptr->ipp1.PORTSC1.BIT.CurrentConnectStatus)
                {
                    ptr->ipp1->USBINTR = 0x00000000; /* Disable interrupt */
                    r_usb_hstd_hci_port_reset(ptr);
                    usb_cpu_delay_xms(15000);
                }
                ...
                ptr->ipp1->USBINTR = 0x00000007; /* PortChange, USBError, USBInt */
            }
  #else
            ptr->ipp->PORTSC1_b.SUSPEND = 1;
            if (1 == ptr->ipp->PORTSC1_b.CCSTS)
            {
                ptr->ipp->USBINTR = 0x00000000; /* Disable interrupt */
                r_usb_hstd_hci_port_reset(ptr);
                usb_cpu_delay_xms(15000);       /* wait 15sec */
...
void r_usb_hstd_hci_wait_time (usb_utr_t * ptr, uint32_t ms)
{
    uint32_t prv;
