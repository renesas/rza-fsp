...
void (* g_usb_callback[])(usb_utr_t *, uint16_t, uint16_t) USB_BUFFER_PLACE_IN_SECTION =
{
    /* PCDC, PCDCC */
#if defined(USB_CFG_PCDC_USE)
...
#if USB_IP_EHCI_OHCI == 1
    epnum = R_USB_HstdConvertEndpointNum(p_ctrl->module_number, pipe);
    ...
    if (USB_CLASS_INTERNAL_PVND < (usb_class_internal_t) (p_ctrl->type))
    {
        ...
        idx =
            (uint8_t) (((uint8_t) (((usb_class_internal_t) p_ctrl->type - USB_CLASS_INTERNAL_HCDC) * 8) +
                        (uint8_t) ((p_ctrl->device_address - 1) * 2)) + (uint8_t) dir);
