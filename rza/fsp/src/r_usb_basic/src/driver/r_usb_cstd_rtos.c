...
static TaskHandle_t g_pmsc_tsk_hdl;
 ...
 #if defined(USB_CFG_HHID_USE)
  #if (USB_UT_MODE == 0)
static TaskHandle_t g_hhid_tsk_hdl;
  #endif                               /* (USB_UT_MODE == 0) */
 #endif                                /* defined(USB_CFG_PMSC_USE) */
...
usb_rtos_err_t usb_rtos_configuration (void)
{
    ...
    ret = xTaskCreate((TaskFunction_t) r_usb_hstd_mgr_task, /** Entry function of USB MGR task  **/
                      "MGR_TSK",                            /** Name of USB MGR task            **/
                      MGR_STACK_SIZE,                       /** Stack size in words             **/
                      NULL,                                 /** Task's parameter                **/
                      MGR_TSK_PRI,                          /** Task's priority                 **/
    ...
    #if defined(USB_CFG_HHID_USE)
    ret = xTaskCreate((TaskFunction_t) r_usb_hhid_task, /** Entry function of USB HHID task  **/
                      "USB_HHID_TSK",                   /** Name of USB HHID task            **/
                      HHID_STACK_SIZE,                  /** Stack size in words              **/
                      NULL,                             /** Task's parameter                 **/
                      HHID_TSK_PRI,                     /** Task's priority                  **/
                      &g_hhid_tsk_hdl);                 /** Task handler for use later       **/
    if (pdPASS != ret)
    {
        err = UsbRtos_Err_Init_Tsk;
