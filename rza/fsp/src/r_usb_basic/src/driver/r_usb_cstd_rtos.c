...
/** Declare a mailbox handler for the created tasks **/
 #if USB_IP_EHCI_OHCI == 0
static rtos_mbx_id_t g_hcd_mbx_hdl USB_BUFFER_PLACE_IN_SECTION;
 #else                                 /* #if USB_IP_EHCI_OHCI == 0 */
...
static rtos_mbx_id_t g_hub_mbx_hdl USB_BUFFER_PLACE_IN_SECTION;
 #if (USB_UT_MODE == 0)
  #if USB_IP_EHCI_OHCI == 0 || (BSP_CFG_RTOS == 1)
...
/** Declare a memory handler for the created tasks **/
 #if (BSP_CFG_RTOS == 2)
  #if USB_IP_EHCI_OHCI == 0
static rtos_mem_id_t g_hcd_mpl_hdl USB_BUFFER_PLACE_IN_SECTION;
  #else                                /* #if USB_IP_EHCI_OHCI == 0 */
...
static uint8_t g_rtos_usb_hcd_mbx[sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)];
   #else
static uint8_t g_rtos_usb_hci_mbx[sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)];
   #endif
...
usb_rtos_err_t usb_rtos_configuration (void)
{
...

    {
        ...
        /** USB HCD task **/
        ret =
            tx_queue_create(&g_hcd_mbx_hdl,
                            "USB_HCD_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hcd_mbx[0],
        ...
        /** USB HCI task **/
        ret =
            tx_queue_create(&g_hci_mbx_hdl,
                            "USB_HCI_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hci_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB MGR task **/
        ret =
            tx_queue_create(&g_mgr_mbx_hdl,
                            "USB_MGR_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_mgr_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB HUB task **/
        ret =
            tx_queue_create(&g_hub_mbx_hdl,
                            "USB_HUB_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hub_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB Internal Communication mailbox **/
        ret =
            tx_queue_create(&g_cls_mbx_hdl,
                            "USB_CLS_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_cls_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB Host MSC **/
        ret =
            tx_queue_create(&g_hmsc_mbx_hdl,
                            "USB_HMSC_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hmsc_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB Host MSC (for class request) **/
        ret =
            tx_queue_create(&g_hmsc_req_mbx_hdl,
                            "USB_HMSC_REQ_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hmsc_req_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB Host CDC **/
        ret =
            tx_queue_create(&g_hcdc_mbx_hdl,
                            "USB_HCDC_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hcdc_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        ret =
            tx_queue_create(&g_hhid_mbx_hdl,
                            "USB_HHID_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hhid_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        /** USB Host Printer **/
        ret =
            tx_queue_create(&g_hprn_mbx_hdl,
                            "USB_HPRN_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_hprn_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
        ...
        g_mbx_table[1]  = &g_mgr_mbx_hdl;                    /* A mailbox handler of USB HCD task */
        g_mbx_table[2]  = &g_mgr_mbx_hdl;                    /* A mailbox handler of USB HCD task */
        g_mbx_table[3]  = &g_hcdc_mbx_hdl;                   /* A mailbox handler of USB MGR task */
        g_mbx_table[4]  = &g_hub_mbx_hdl;                    /* A mailbox handler of USB HUB task */
        g_mbx_table[5]  = &g_cls_mbx_hdl;                    /* A mailbox handler of USB internal communication */
        g_mbx_table[6]  = &g_pcd_mbx_hdl;                    /* A mailbox handler of USB PCD task */
        g_mbx_table[7]  = &g_hmsc_mbx_hdl;                   /* A mailbox handler of USB HMSC */
        g_mbx_table[8]  = &g_hmsc_req_mbx_hdl;               /* A mailbox handler for class request (via PIPE0) of USB HMSC */
        g_mbx_table[9]  = &g_hhid_mbx_hdl;                   /* A mailbox handler of USB HCDC task */
        g_mbx_table[10] = &g_hhid_mbx_hdl;                   /* A mailbox handler of USB HHID task */
        g_mbx_table[11] = &g_pmsc_mbx_hdl;                   /* A mailbox handler of USB PMSC task */

        /* MGR Task */
        ret = tx_thread_create(&g_mgr_tsk_hdl,               /** Pointer to a thread control block.              **/
                               "MGR_TSK",                    /** Pointer to the name of the thread.              **/
                               usb_hstd_mgr_task,            /** Entry function of USB HCD task                  **/
                               (uint32_t) (uintptr_t) NULL,  /** Task's parameter                                **/
                               (void *) &g_usb_mgr_stack[0], /** Starting address of the stack fs memory area.   **/
                               MGR_STACK_SIZE,               /** Number bytes in the stack memory area.          **/
                               MGR_TSK_PRI,                  /** Task's priority                                 **/
                               MGR_TSK_PRI,                  /** Task's priority                                 **/
...

    {
        ...
        /** USB PCD task **/
        ret =
            tx_queue_create(&g_pcd_mbx_hdl,
                            "USB_PCD_MBX",
                            TX_1_ULONG,
                            &g_rtos_usb_pcd_mbx[0],
                            (sizeof(ULONG) * (TX_1_ULONG) *(QUEUE_SIZE)));
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
...
usb_rtos_err_t usb_rtos_delete (void)
{
    ...
    #else                              /* #if USB_IP_EHCI_OHCI == 0 */
        ret = tx_queue_delete(&g_hci_mbx_hdl);
        if (TX_SUCCESS != ret)
        {
            err = UsbRtos_Err_Init_Mbx;
