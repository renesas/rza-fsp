...
extern rtos_sem_id_t SemaphoreHandleRead;
...
usb_er_t R_USB_HstdMgrOpen (usb_utr_t * ptr)
{
  ...
  #elif BSP_CFG_RTOS == 1
    tx_semaphore_create(&SemaphoreHandleRead, "USB_FSP_SEM_READ", 0);
  #endif                               /* #if (BSP_CFG_RTOS == 2) */
 #endif                                /* #if defined(USB_CFG_HMSC_USE) */
    ...
    g_usb_hstd_device_addr[ptr->ip]  = USB_DEVICE_0; /* Device address */
    g_usb_hstd_device_num[ptr->ip]   = 0;            /* Device driver number */

    for (i = 0; i < (USB_MAXDEVADDR + 1U); i++)
    {
        p_driver = &g_usb_hstd_device_drv[ptr->ip][i];
 ...
 #else
    usb_cstd_set_task_pri(USB_HCI_TSK, USB_PRI_1);
...
void R_USB_HstdMgrClose (void)
{
  ...
  #elif (BSP_CFG_RTOS == 1)
    tx_semaphore_delete(&SemaphoreHandleRead);
  #endif
 #endif                                /*  #if defined(USB_CFG_HMSC_USE) */
}                                      /* End of function R_USB_HstdMgrOpen() */
...
{
    return usb_hstd_convert_epnum(module_number, pipe);
