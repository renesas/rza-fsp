...
fsp_err_t R_USB_Open (usb_ctrl_t * const p_api_ctrl, usb_cfg_t const * const p_cfg)
{
    ...
    g_usb_class_info[p_ctrl->module_number][USB_MAXDEVADDR + 1] = p_ctrl->type;
#if (BSP_CFG_RTOS == 1)
    ...
    switch ((usb_class_internal_t) p_ctrl->type)
    {
 ...
 #else                                 /*  #if (BSP_CFG_RTOS == 1) */
            FSP_ERROR_RETURN(!((USB_SPEED_LS == p_cfg->usb_speed) || (NULL == p_cfg->p_usb_reg)), FSP_ERR_USB_PARAMETER)
 #endif /*  #if (BSP_CFG_RTOS == 1) */
        ...
        case USB_CLASS_INTERNAL_HMSC:
        {
            ...
            FSP_ERROR_RETURN(!((USB_SPEED_HS == p_cfg->usb_speed) && (USB_IP1 != p_ctrl->module_number)),
                             FSP_ERR_USB_PARAMETER)
  #elif (BSP_MCU_GROUP_RZT2M == 1) || (BSP_MCU_GROUP_RZA_USB == 1)
            FSP_ERROR_RETURN(USB_SPEED_HS == p_cfg->usb_speed, FSP_ERR_USB_PARAMETER)
  #else                                /* defined(BSP_MCU_GROUP_RA6M3) */
            FSP_ERROR_RETURN(USB_SPEED_HS != p_cfg->usb_speed, FSP_ERR_USB_PARAMETER)
  #endif /* defined(BSP_MCU_GROUP_RA6M3) */
 ...
 #endif                                /* (BSP_MCU_GROUP_RZT2M) && (BSP_MCU_GROUP_RZA3UL)  */
        }
    ...
    else
    {
        ...
        if (FSP_SUCCESS == err)
        {
            ...
            if (USB_IP0 == p_ctrl->module_number)
            {
                usb_peri_usbx_initialize((uint32_t) R_USB01_BASE);
            }
 #endif                                /* #if (BSP_CFG_RTOS == 1) */
...
#if defined(USB_CFG_HHID_USE)
 #if (BSP_CFG_RTOS != 1)
            g_usb_callback[USB_CLASS_INTERNAL_HHID * 2]       = usb_hhid_read_complete;
            g_usb_callback[(USB_CLASS_INTERNAL_HHID * 2) + 1] = usb_hhid_write_complete;
 #endif                                /*  (BSP_CFG_RTOS != 1)  */
#endif                                 /*  defined(USB_CFG_HHID_USE)  */
        }
        else
...
fsp_err_t R_USB_Close (usb_ctrl_t * const p_api_ctrl)
{
    ...
    p_ctrl->type = g_usb_class_info[p_ctrl->module_number][USB_MAXDEVADDR + 1];
#if USB_IP_EHCI_OHCI == 1
 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
    ...
    if (FSP_SUCCESS == ret_code)
    {
        ...
        else
        {
            ...
            usb_peri_usbx_uninitialize((uint32_t) R_USB01_BASE);
 #endif                                /* #if (BSP_CFG_RTOS == 1) */
#endif  /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
        }
...
fsp_err_t R_USB_Read (usb_ctrl_t * const p_api_ctrl, uint8_t * p_buf, uint32_t size, uint8_t destination)
{
    ...
    else
    {
        ...
        p_ctrl->type           = g_usb_class_info[p_ctrl->module_number][p_ctrl->device_address];
 #if defined(USB_CFG_HCDC_USE)
...
#if USB_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(!(((NULL == p_buf)) || (USB_NULL == size)))
    FSP_ERROR_RETURN((((uint32_t) (uintptr_t) p_buf & 0x03) == 0), FSP_ERR_USB_PARAMETER)
...
#if USB_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(!((NULL == p_ctrl) || (NULL == p_pipe)))
                                     ...
                                     uint8_t            device_address)
{
    ...
    FSP_ASSERT(p_api_ctrl)
    FSP_ASSERT(p_setup)
    FSP_ASSERT(!((NULL == p_buf) && (0 != p_setup->request_length)))
...
fsp_err_t R_USB_OtgCallbackSet (usb_ctrl_t * const p_api_ctrl, usb_otg_callback_t * p_callback)
{
    ...
    FSP_ASSERT((void *) USB_NULL != p_callback);
