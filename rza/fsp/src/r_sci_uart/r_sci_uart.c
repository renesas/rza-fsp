...
#define SCI_UART_NUM_DIVISORS_ASYNC             (7U)
...
void sci_uart_rxi_isr(IRQn_Type const irq);

void r_sci_uart_read_data(sci_uart_instance_ctrl_t * const p_ctrl, uint32_t * const p_data);

void sci_uart_eri_isr(IRQn_Type const irq);

#endif

#if (SCI_UART_CFG_TX_ENABLE)
void sci_uart_txi_isr(IRQn_Type const irq);
void sci_uart_tei_isr(IRQn_Type const irq);
...
fsp_err_t R_SCI_UART_Open (uart_ctrl_t * const p_api_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    R_BSP_MODULE_CLKON(FSP_IP_SCI, p_cfg->channel);
    R_BSP_MODULE_RSTOFF(FSP_IP_SCI, p_cfg->channel);
...
fsp_err_t R_SCI_UART_Close (uart_ctrl_t * const p_api_ctrl)
{
    ...
    R_BSP_MODULE_CLKOFF(FSP_IP_SCI, p_ctrl->p_cfg->channel);
                                                ...
                                                uint32_t                         sci_buffer_address)
{
    ...
    *p_transfer_reg = sci_buffer_address;
    ...
    dmac_extended_info_t * p_extend_info = (dmac_extended_info_t *) p_transfer->p_cfg->p_info->p_extend_info;
